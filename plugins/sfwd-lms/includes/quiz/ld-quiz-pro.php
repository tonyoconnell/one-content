 ...
 */
require_once dirname( __DIR__ ) . '/vendor/wp-pro-quiz/wp-pro-quiz.php';
...
class LD_QuizPro {
	...
	function checkAnswers( $data ) {
		...
		foreach ( $data['responses'] as $question_id => $info ) {
			...
			$question_index++;
			$answer_pointed_activated = $this_question->isAnswerPointsActivated();

			switch ( $questionData['type'] ) {
				case 'free_answer':
					$correct = (strtolower( $response ) == strtolower( $questionData['correct'][0] ));
				...
				case 'multiple':
					...
					foreach ( $questionData['correct'] as $answerIndex => $answerChecked ) {
						
						$checked = $questionData['correct'][ $response[ $answerIndex ] ];
						...
						if ( $answer_pointed_activated ){
							 ...
							 */
							if ( ! empty( $answerChecked ) && ! empty( $response[ $answerIndex ] ) ) {
								//Correct
								$correct = true;
								$r[ $answerIndex ] = true;
							...
							} elseif ( empty( $answerChecked ) && ! empty( $response[ $answerIndex ] ) ) {
								$r[ $answerIndex ] = false;
								$points = 0;
								break;
							} elseif ( ! empty( $answerChecked ) && empty( $response[ $answerIndex ] ) ) {
				...
				case 'single':
					foreach ( $questionData['correct'] as $answerIndex => $answerChecked ) {
						if ( empty( $response[ $answerIndex ] ) ) {
							continue;
						}
					
						if ( ! empty( $questionData['diffMode'] ) || ! empty( $answerChecked ) ) {
							//DiffMode or Correct
						...
						if ( ! empty( $answerChecked) || ! empty( $questionData['disCorrect'] ) ) {
							//Correct
							$correct = true;
						}
					}
				...
				case 'matrix_sort_answer':
					...
					foreach ( $questionData['correct'] as $answerIndex => $answer ) {
						if ( ! isset( $response[ $answerIndex ] ) || $response[ $answerIndex ] != $answer ) {
							$correct = false;
						} else {
							if ( is_array( $questionData['points'] ) ) {
								$points += $questionData['points'][ $answerIndex ];
						...
						$statisticsData->{$answerIndex} = @$response[ $answerIndex ];
					}
				...
				case 'cloze_answer':
					...
					foreach ( $questionData['correct'] as $answerIndex => $correctArray ) {
						if ( ! isset( $response[ $answerIndex ] ) || ! in_array( $response[ $answerIndex ], $correctArray ) ) {
							$correct = false;
							if ( ! $quiz->isDisabledAnswerMark() ) {
