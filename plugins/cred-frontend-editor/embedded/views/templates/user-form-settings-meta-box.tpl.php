...
<fieldset class="cred-fieldset">
    ...
    <p class='cred-explain-text'><?php _e('Select allowed User Role', 'wp-cred'); ?>
        <?php
        if ($settings['type'] == 'edit') {
            echo _e("(No items selected means all user role are allowed)", 'wp-cred');
        }
        ?>:
    </p>
