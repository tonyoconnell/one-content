        ...
        if (isset($fields['form_settings'])) {
            ...
            if (!isset($s->form)) {
                ...
                $setts->form = array(
                    ...
                    'autogenerate_password_scaffold' => isset($s->autogenerate_password_scaffold) ? $s->autogenerate_username_scaffold : 0,
...
//=================== GENERAL (CUSTOM) POST HANDLING METHODS ====================================================
    ...
    public function setAttachedData($user_id, $data) {
        return update_user_meta(intval($user_id), '__cred_notification_data', $data); // serialize
    }

    public function removeAttachedData($user_id) {
        return delete_user_meta(intval($user_id), '__cred_notification_data');
    }

    public function getAttachedData($user_id) {
        return get_user_meta(intval($user_id), '__cred_notification_data', true); // unserialize
    }
    ...
    public function addUser($userdata, $usermeta) {
        ...
        if (!is_wp_error($user_id)) {
            ...
            foreach ($usermeta as $meta_key => $meta_value) {
                if (is_array($meta_value) && !$usermeta['info'][$meta_key]['save_single']) {
    ...
    public function updateUser($userdata, $usermeta) {
        $allowed_tags = $allowed_protocols = array();
        $this->setAllowed($allowed_tags, $allowed_protocols);
