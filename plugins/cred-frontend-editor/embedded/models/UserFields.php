...
final class CRED_User_Fields_Model extends CRED_Abstract_Model implements CRED_Singleton {
    ...
    public function getFields($autogenerate = array(), $add_default = true, $localized_message_callback = null) {
        // ALL FIELDS
        $fields_all = array();
        // POST FIELDS
        $fields = array();
        ...
        if (!empty($usermetas)) {
            ...
            foreach ($usermetas as $n => $metas) {
                ...
                $fields[$metas['meta_key']]['type'] = 'textfield';
                $fields[$metas['meta_key']]['name'] = ucwords(str_replace("_", " ", $metas['meta_key']));
                ...
                $fields[$metas['meta_key']]['post_labels'] = ucwords(str_replace("_", " ", $metas['meta_key']));
        ...
        if ($add_default) {
            ...
            if (isset($autogenerate['username']) && $autogenerate['username'] == 'false') {
            ...
            if (isset($autogenerate['password']) && $autogenerate['password'] == 'false') {
            ...
            $user_fields['user_url'] = array('post_type' => 'user', 'post_labels' => __('Url', 'wp-cred'), 'id' => 'user_url', 'wp_default' => true, 'slug' => 'user_url', 'type' => 'textfield', 'name' => __('Url', 'wp-cred'), 'description' => 'Url', 'data' => array(/* 'repetitive' => 0, 'validate' => array ( 'required' => array ( 'active' => 1, 'value' => true, 'message' => __('This field is required','wp-cred') ) ), 'conditional_display' => array ( ), 'disabled_by_type' => 0 */));
        }
