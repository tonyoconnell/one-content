            ...
            if (isset($_POST['_recaptcha'])) {
                ...
                (
                    ...
                    if (empty($_POST['recaptcha_response_field'])) {
                        $zebraForm->add_top_message($formHelper->getLocalisedMessage('enter_valid_captcha'));
                        $zebraForm->add_field_message($formHelper->getLocalisedMessage('enter_valid_captcha'), 'user_pass');
                        $result = false;
                        //return $result;
                    }
                }
            ...
            if ($post_type == 'user') {
                if (isset($_POST['user_pass'])) {
                    ...
                    if (empty($_POST['user_pass']) ||
                            (!empty($_POST['user_pass']) && $_POST['user_pass'] != $_POST['user_pass2'])) {
    ...
    private function CRED_user_save($user_role, $user_id = null) {
        ...
        if ($all_ok) {
            ...
            if ($form_type == 'edit' && isset($user_id)) {
                $new_user_id = $model->updateUser($user, $fields);
            } else {
                $new_user_id = $model->addUser($user, $fields);
            }

            //cred_log(array('fields'=>$fields, 'info'=>$fieldsInfo, 'removed'=>$removed_fields));
            if (false && is_int($new_user_id) && $new_user_id > 0) {
