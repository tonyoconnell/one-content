...
if (!class_exists("CredUserFormCreator", false)) {
    ...
    class CredUserFormCreator {
         ...
         */
        public static function cred_create_form($name, $mode/* new/edit */, $user_type = array('subscriber'), $autogenerate_un = false, $autogenerate_p = false, $post_type = 'user') {
            ...
            if (empty(self::$_created) && !in_array($name, self::$_created)) {
                ...
                $fields_all = $fields_model->getFields();
...
//                }
                ...
                $groups_out = '';
                $out = '';
                foreach ($fields_all['form_fields'] as $f => $fields) {
                    if ($fields['cred_builtin'] == 1)
                        continue;
                    $nlcnt++;
